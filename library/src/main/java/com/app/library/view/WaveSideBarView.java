...
import com.app.library.entity.CityIndexBean;
...
public class WaveSideBarView extends View {
    ...
    // 渲染字母表
    private List<CityIndexBean> mLetters = new ArrayList<>();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mItemHeight = mLetters.size() == 0 ? 0 : (mHeight - mPadding) / mLetters.size();
    ...
    private void drawLetters(Canvas canvas) {
        ...
        for (int i = 0; i < mLetters.size(); i++) {
            ...
            if (i == mChoose) {
                ...
                canvas.drawText(mLetters.get(i).getFirstLetter(), mPosX, posY, mLettersPaint);
            }
        }
    ...
    private void drawChooseText(Canvas canvas) {
        if (mChoose != -1) {
            ...
            mLettersPaint.setTextSize(mTextSize);
            mLettersPaint.setTextAlign(Paint.Align.CENTER);
            canvas.drawText(mLetters.get(mChoose).getFirstLetter(), mPosX, mPosY, mLettersPaint);

            // 绘制提示字符
            if (mRatio >= 0.9f) {
                String target = mLetters.get(mChoose).getFirstLetter();
    ...
    public List<CityIndexBean> getLetters() {
        return mLetters;
    }

    public void setLetters(List<CityIndexBean> letters) {
        this.mLetters = letters;
        invalidate();
    }

    public interface OnTouchLetterChangeListener {
        void onLetterChange(CityIndexBean letter);
    }
}